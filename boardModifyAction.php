...
$oBoardModifyModel = new BoardModel();

$modifyResult = $oBoardModifyModel->updateBoardFromBoardNum($num,$subject,$content,$date,$upfile_name,$copied_file_name,$videoLink);
if($modifyResult){
    echo "<script>alert('게시물을 수정했습니다.');";
    echo "window.location.replace('board.php');</script>";
    exit;
}else{
    echo "<script>alert('게시물 수정에 실패했습니다..');";
    echo "window.location.replace('board.php');</script>";
    exit;
}
